...
const HeaderComponent =() =>{
   ...
   const cartItems=useSelector(store => store.cart.items);
    return (
      ...
      <div className="flex p-2 justify-between bg-pink-50 shadow-lg   w-full  fixed -my-3 ">
        <img
    alt="logo"
    className="h-20"  
    ...
    />
        <div >
          ...
          {/* <h1>{user.name}</h1> */}
        <h1 className="my-10">{isonline?"online":"offline"}</h1>  

        {islogin?
        <button onClick={()=>islogout(false)}>logout</button>
        :<button onClick={()=>islogout(true)}>login</button>
